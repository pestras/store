...
export class MapStore<T = any> {
   ...
   * @returns **T**
   */
  protected insert(doc: T, replace = false, silence?: boolean): T {
    ...
    this.onChange && !silence && this.onChange([doc], 'insert');
   ...
   * @returns **T[]**
   */
  protected insertMany(docs: T[], replace = false, silence?: boolean): T[] {
    ...
    this.onChange && !silence && this.onChange(inserted, 'insert');
   ...
   * @returns **T**
   */
  protected update(id: string, update: Partial<T>, silence?: boolean): T {
    let map = this.container;
   ...
   * @returns **T[]**
   */
  protected updateMany(ids: string[], update: Partial<T>, silence?: boolean): T[];
  /**
  ...
  protected updateMany(filter: string[] | ((doc: T) => boolean), update: Partial<T>, silence?: boolean): T[] {
    let map = this.container;
    let updated: T[] = [];
    ...
    this.onChange && !silence && this.onChange(updated, 'update');
    this._dataSub.next(map);
   ...
   * @returns **T[]**
   */
  protected bulkUpdate(updates: Partial<T>[], silence?: boolean): T[] {
    let map = this.container;
    let updated: T[] = [];
   ...
   * @returns **T**
   */
  protected removeOne(id: string, silence?: boolean): T {
    let map = this.container;
    let doc = map.get(id);
    ...
    map.delete(id);
    this.onChange && !silence && this.onChange([doc], 'remove');
    this._dataSub.next(map);
   ...
   * @returns **T[]**
   */
  protected removeMany(ids: string[], silence?: boolean): T[]
  /**
  ...
  protected removeMany(filter: string[] | ((doc: T) => boolean), silence?: boolean): T[] {
    let map = this.container;
    let removed: T[] = [];
